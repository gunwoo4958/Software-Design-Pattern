...
public class RemoteLoader {
	...
	public static void main(String[] args) {
		...
		LightDimUpCommand DimUP = new LightDimUpCommand(light);
		StereoOnCommand stereoOn = new StereoOnCommand(stereo);
		TVOnCommand tvOn = new TVOnCommand(tv);
		HottubOnCommand hottubOn = new HottubOnCommand(hottub);

		LightOffCommand lightOff = new LightOffCommand(light);
		LightDimDownCommand DimDown = new LightDimDownCommand(light);
		...
		remoteControl.setCommand(5, DimUP, DimDown);
		...
		do {
			...
			switch (input) {
				...
				case "10":
					System.out.println("--- Light Dim Up---");
					remoteControl.onButtonWasPushed(5);
					break;
				case "11":
					System.out.println("--- Light Dim Down---");
